...
const DialogCard = ({ trigger, content }: DialogCardProps) => (
  <Dialog.Root>
    ...
    <Dialog.Portal>
      ...
      <Dialog.Content className="fixed left-1/2 top-1/2 max-h-full w-[90vw] max-w-7xl -translate-x-1/2 -translate-y-1/2 rounded-lg bg-blue-light p-[25px] shadow-[hsl(206_22%_7%_/_35%)_0px_10px_38px_-10px,_hsl(206_22%_7%_/_20%)_0px_10px_20px_-15px] focus:outline-none data-[state=open]:animate-contentShow">
        <div>
          {content}
          <Dialog.Close asChild>
            <button className="flex justify-end w-full" aria-label="Close">
              <LuX size={26} />
            </button>
          </Dialog.Close>
        </div>
